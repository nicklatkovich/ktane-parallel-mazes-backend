...
export class SocketClient {
    ...
    const log = (str: string) => console.log(`[${new Date().toISOString()}] ${this.id} ${str}`);
    log("connected");
		this.ws.on("close", () => {
      log("disconnected");
      for (const handler of this.disconnectHandlers) handler();
        ...
        if (method !== "ping") log(`.${method}(${JSON.stringify(args)}) => ${JSON.stringify(result)}`);
				ws.send(JSON.stringify({ type: "success", id, request: json, result }));
			} catch (error) {
				if (!(error instanceof ClientError)) throw error;
        const reason = error.data;
        log(`.${method}(${JSON.stringify(args)}) exception: ${JSON.stringify(reason)}`);
